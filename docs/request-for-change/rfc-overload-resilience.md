...
| black-box tool truly wedged → bounded, partial result kept | `test_tool_interactive_policy.py` | `UNKNOWN` past budget → that call cancelled, partial output in `progress_json`, task `recovering` |
...
**2000-task experiment** (`scripts/taskq_experiment.py`, fake harness + real
scheduler, admission, controller and store): record accepted, unique completed,
failed, cancelled, remaining, duplicates, effective-cap timeline, peak procs/fds/
RSS, recovery durations, manual interventions (must be 0), machine spec, and
response latency of `/api/tasks` at full load. Fault-injection hooks
(`KIROCREW_TASKQ_FAULTS=slow_init:30s,enomem@t+120s,daemon_kill@t+300s`) are
read only when the env var is set and are refused when `KIROCREW_HOME` is the
default path.
...
API); `test_overload_experiment_smoke.py` drives the fake-harness experiment at
N=100 with every fault knob. The measured 2000/500-task runs are in
`docs/reports/overload-resilience-experiment.md`.

| Scenario | Test(s) |
|---|---|
| 2000-task burst | `test_taskq_admission_integration.py::test_2000_submissions_all_complete_window_never_exceeds_64`, `test_overload_experiment_smoke.py::test_baseline_two_sessions_and_system_lane_all_complete` |
| automatic down-scaling | `test_adaptive_policy.py`, `test_overload_experiment_smoke.py::test_controller_decreases_under_injected_timeouts_and_recovers` |
...
| cross-process restart | `test_taskq_reconcile.py`, `test_taskq_admission_integration.py::test_queued_rows_survive_restart_and_redispatch`, `test_overload_acceptance.py::test_a12_gateway_restart_during_a_wait_rebuilds_identity_and_links`, `test_overload_experiment_smoke.py::test_restart_mid_run_reconciles_and_adopts_a_late_start` |
| outage longer than the old stub budget | `test_stub_broker_reconnect.py`, `test_overload_acceptance.py::test_i12_many_waiting_tasks_and_normal_tasks_coexist_bounded`, `test_overload_experiment_smoke.py::test_outage_parks_runs_on_one_scope_and_nothing_is_severed`; defect D2 pinned by `test_d2_gatewayd_outage_longer_than_a_minute_must_not_fail_the_scope` (strict xfail) |
...
| Interactive §9 rows 1-12 | `test_overload_acceptance.py::test_i01` .. `test_i12` (each docstring names the owner tests). Rows 2, 6 and 7 (a real TTY via PTY, many terminals waiting without cross-talk, stdin-closed / invalid-handle errors) are properties of the controlled terminal and land with its follow-up PR |
| 2000-task experiment | `scripts/experiments/overload_2000.py` (`taskq_experiment.py` above is this file's final name); `test_overload_experiment_smoke.py` |
...
| 3 | G fairness | lanes (`system` lane for cron/hook), `child_reserve`, checkpoint-pause interface behind a default-off flag | ~0.4k | `agent.child_reserve_slots=1`, `agent.interactive_reserve_slots=1`, `agent.lane_weights={}`, `agent.parent_checkpoint_pause=false` | taskq, subagent |
...
| 3 | J experiment | `scripts/taskq_experiment.py`, 2000-task run, fault hooks, report in the PR body | script + report | `KIROCREW_TASKQ_FAULTS` (env, refused on the default home) | taskq |
...
- **Q3.** Checkpoint-pause for dedicated-runtime parents depends on continuable
  ...
  **Decision:** implement `child_reserve` (1 slot) now. Checkpoint-pause lands as
  interface plus tests behind `agent.parent_checkpoint_pause=false` (default off,
  no behaviour change). Reversal/enablement: flip that flag once fidelity is
  proven; until then residency exhaustion by waiting parents is a hard admission
  refusal for new parents, as §6 already states for the degraded case.
