...
| [rfc-member-inbox-model.md](rfc-member-inbox-model.md) | `draft` | Nothing. Verified at `c0307ef08`: a crew member is still a long-lived `member-<slug>` slot; every stimulus (human, cron, nudge, `session_send`, worker completion) is an injected turn on it; a member calling `session_send` on another member's thread is refused `not_creator` by the generic creator fence at the tail of `authorize_target` (`dashboard/session_control.py`) because a DM slot is minted with an empty `_created_by`; there is no inbox, no per-member scheduler, no envelope type, and `send_to_target` has no rate limit or hop count |
