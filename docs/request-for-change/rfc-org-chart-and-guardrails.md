...
This proposal accompanies the executable organization prototype. The owning
runtime contract is [crew-mode](../system-specs/modules/crew-mode.md).
...
member. Members outside the organization remain ordinary Crew Members.
...
After a gateway restart, an in-flight organization turn becomes interrupted.
The system preserves its identity and evidence and requires a deliberate retry:
it cannot assume that an external action failed merely because completion was
not recorded. Agent-requested retries are bounded; the owner can inspect and
