...
A locked or read-only database raises and aborts the config write; the next load
retries the whole migration. Only a missing `memory_meta` table is tolerated, because
that is a legacy V1 file with no owner row to rename.
