...
both halves of the re-key (a row created with a well-formed id has none) -- never the
...
not copied into the base file. A warning names the key to rename in the overlay.
...
read-only database propagates and aborts the pass. The DM binding, the rules payload
