...
stub is held before a `capacity` refusal and deliberately equals the stub's own
reconnect budget; `initialize_timeout_secs` (10) bounds a fresh backend's first
...
class AgentConfig:
    ...
    parent_checkpoint_pause: bool = False  # close a waiting dedicated-runtime parent at a checkpoint and resume it via the continuable path (ParentPauser interface); off = the waiting parent keeps its resident process
    ...
    parent_checkpoint_pause: bool = False      # on-path of the ParentPauser seam (close a waiting dedicated-runtime parent at a checkpoint, resume via the continuable-session path); off keeps the no-op pauser whatever is installed. Live
