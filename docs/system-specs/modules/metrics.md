...
| `kirocrew.mcp.gateway.liveness_overloaded` | counter | — (deliberately none: the connection and task counts that describe the overload grow with load and belong in the warning line, never in a series key) | `mcp_gateway/manager.py::GatewayManager._liveness_probe_loop`, one increment per watchdog cycle in which the daemon missed `_LIVENESS_MAX_CONSECUTIVE_FAILURES` liveness pings but its own self-report (`mcp_gateway/self_report.py::read_last_probe` over `logs/gatewayd_zombie_diagnostic.jsonl`) vouched for it — a `probe` record from the SAME pid, `is_serving` true, stamped within `_SELF_REPORT_FRESH_SECS` of now — so the supervisor kept it instead of SIGKILLing it. Each increment is a kill the daemon's own word averted: the daemon was overloaded, not dead. A rising rate is a daemon running near saturation (each increment would otherwise have severed every session's MCP transport), which is capacity pressure to act on, not instability that healed. See [mcp-gateway-daemon-lifecycle](mcp-gateway-daemon-lifecycle.md#overload-is-not-death). |
...
| `kirocrew.taskq.depth` | histogram (1) | `state` (one of the 13 `taskq.model` states) | `dashboard/session_health.py::SessionHealthMonitor._sample_metrics`, one observation per state per health computation: the durable queue's depth as the sampler saw it. The aggregator's last/max reading per state is the gauge. |
