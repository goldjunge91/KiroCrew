...
`kiro_crew.agent_sdk.projected_session_mcp_servers(agent, *, work_dir=None)` is the
synchronous provider-neutral entry to the existing session MCP projection. It
returns the same filtered list of dictionaries and preserves resolver errors;
async callers offload it. Projection itself neither grants private authority nor
