...
|---|---|---|
| answer + `late_adopter` returned True | `adopted` | the session is finished through the same tail as a direct start (`_finish_create_session`: queue, handle, mode activation, drain) and handed to the adopter; a late-adopted session has no buffered init frames |
...
released exactly once on every path; no collector for a pre-wire timeout;
`run.py` never calls `get_or_create` on congestion and continues on an
adopted late session.
