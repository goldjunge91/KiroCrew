...
- **`ParentPauser` (Q3 interface).** Protocol `can_pause(info)`,
  `async pause(info)`, `async resume(info)`. `parent_pauser()` returns
  `NoopParentPauser` unless `agent.parent_checkpoint_pause` is true, in which
  case an installed `manager._parent_pauser` is used (nothing installed →
  still the no-op). On-path: `yield_slot` for a `waiting_children` record
  calls `_maybe_pause_parent` → `pauser.pause(info)` as a task
  (`info._pause_pending` / `info._paused`); `resume_grant` calls
  `_maybe_resume_paused_parent` → `pauser.resume(info)`. No implementation
  ships: a dedicated-runtime parent keeps its resident process and its host
  charge until `session/load` fidelity for a mid-tool-call parent is proven.
...
- **Settings.** `FairnessSettings.from_agent_config(cfg.agent)` (cached on
  ...
  `adaptive_floor`, `parent_checkpoint_pause`.
