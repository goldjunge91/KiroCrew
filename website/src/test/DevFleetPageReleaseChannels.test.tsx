...
describe('DevFleetPage release-channel rows', () => {
  ...
  it('explains the occupied name on the existing row, not as a second row', async () => {
    ...
    expect(screen.queryByTestId('release-channel-placeholder-stable')).not.toBeInTheDocument()
    // The badge states the VERDICT, not two situation fragments. "Reserved name,
    // on a branch" put the meaning only in the hover tooltip, and a tooltip is
    // mouse-only; the reserved-name half is already on screen as the row's own
    // name, so the badge carries what the name cannot.
    const badge = screen.getByText('Not the channel — on a branch')
  ...
  it('gives a resolver failure an ErrorNotice with the agent hand-off, not just a tooltip', async () => {
    // Every other error class on this page goes through ErrorNotice with askAgent.
    // A resolver failure reached the user only as tooltip and cell text, which is
    // not keyboard-reachable and carries no hand-off.
    const broken = {
      ...CHANNELS.stable,
      worktree: null,
      ref: null,
