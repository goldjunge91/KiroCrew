    ...
    <div className="min-w-0 space-y-4 px-4 pb-6 md:px-6">
      ...
      {!data ? <p className="text-[var(--muted)]">{t('organization.loading')}</p> : <>
        <div className="flex flex-wrap items-center gap-3 rounded-xl border border-[var(--border)] p-3">
          ...
          <span className="flex-1 text-sm">{label(data.settings.enabled ? 'running' : 'paused')}</span>
        ...
        {!data.runtime.ready && <Card>
          ...
          <p className="mt-2 text-sm text-[var(--muted)]">{data.runtime.reason}</p>
        </Card>}
        ...
        {active.length > 0 && <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(280px,0.85fr)]">
          ...
          {selected && <Card className="min-w-0 space-y-4">
            ...
            <p className="text-sm">{label(selected.role)} · {t('organization.manager')}: {memberLabel(selected.manager_id)}</p>
            ...
            {selected.manager_id && <label className="block space-y-1 text-sm">
              <span>{t('organization.manager')}</span>
              <SimpleSelect value={selected.manager_id}
                ...
                disabled={busy} aria-label={t('organization.manager')} />
            </label>}
            <div className="flex flex-wrap gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem disabled={reports.length > 0} onSelect={() => { void send('retire', { member_id: selected.id }).catch(() => {}) }}>{t('organization.retire')}</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
            {(selected.role === 'conductor' || selected.role === 'manager') && <form className="space-y-3"
              ...
              <PanelSectionHeader label={t('organization.addMember')} count={reports.length} />
        ...
        <TabsContent value="work"><div className="grid min-w-0 gap-4 xl:grid-cols-2">
            ...
            {selected && <Card>
              <form className="space-y-3" onSubmit={event => {
                ...
                <label className="block space-y-1 text-sm"><span>{t('organization.taskTitle')}</span>
                  <Input required value={title} maxLength={2000} onChange={event => setTitle(event.target.value)} /></label>
          ...
          <div className="space-y-4" aria-live="polite">
            {!data.tasks.length && <Card>{t('organization.noWork')}</Card>}
            {data.tasks.map(task => <Card key={task.id} className="min-w-0 space-y-3">
              ...
              {task.sender === 'owner' && !['accepted', 'cancelled'].includes(task.state) && <div className="space-y-2">
                <textarea aria-label={t('organization.decision')} className={FIELD} rows={2} value={decisions[task.id] || ''}
            ...
            {data.runs.filter(run => ['failed', 'interrupted'].includes(run.state)).slice(0, 10).map(run =>
