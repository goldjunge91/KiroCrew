...
export default function CapabilityRowEditor({ row, operation, disabled, models, onChange }: {
  ...
  return (
    <div className="min-w-0 border-b border-border py-2.5" data-testid={`capability-${row.section}-${row.id}`}>
      ...
      {row.section === 'prompt' && <label className="mt-2 block text-[13px]">{label}
        <textarea aria-label={label} className="block w-full rounded border border-border bg-bg p-2 text-[13px]" rows={4} disabled={locked} value={state === 'removed' ? '' : text} onChange={e => set(e.target.value)} />
      </label>}
