...
9. PRODUCT SHAPE NEEDS A RECORDED DECISION. A product-shape item is
   ...
   - a maintainer's explicit decision on THIS head, recorded as
     ...
     `Clears when:` when the pull request is from a fork.
...
- When unsure, LOWER the concern (prefer CONCERNS over BLOCK), and
  never invent a consequence. The four exceptions are named at the
  tie-breaker below; all four are settled by READING the diff, the
  description and the base RFC list rather than by degree of
  confidence, and there is no fifth.
...
- BLOCK: an item's zero option costs nobody anything; or it duplicates
  ...
  (lens 9) has no recorded decision on the base commit; or the lane
  CANNOT EVALUATE a product-shape item because evidence this contract
  requires is absent -- triggers (c) and (d) below. (A BLOCK fails
  this check and blocks PR readiness, so the red check holds the
  merge until the evidence or the decision is supplied, or a human
  overrides.)
...
meanwhile. Four combinations are settled by reading the diff, the
description and the base RFC list rather than by degree of confidence,
so the tie-breaker does not reach any of them, and there is no fifth.
...
non-draft on the base, or `/ai-review override first-principles <head
sha>: <decision>` from a maintainer. A well-argued description does
not lower this: the argument is the proposal.
(d) CANNOT EVALUATE -- REQUIRED EVIDENCE ABSENT: an inventory item is
PRODUCT SHAPE (the same class lens 9 gates: a changed default, a change
to what a first-class loop, monitor, agent, skill or command does by
default, a removed or replaced user-facing capability) and the
evidence THIS contract requires to judge it is absent from the diff,
the description and the repository: the harm it removes is asserted
with nothing to point at; a removal's zero consumers were asserted
rather than counted; a measured-cost claim carries no number. You
cannot judge what you were not shown, and a verdict you cannot reach
must not read as advisory: the punchline opens `cannot evaluate:
missing <X>` and `Clears when:` names the artifact. Absence is read,
not judged. This exit is scoped to product shape on purpose: an
ordinary fix with thin provenance stays an `inherited` item at
CONCERNS, as lens 3 already says -- (d) does not widen that.
...
line and the base RFC list entry that is absent or says `draft`. For
(d), the reading is the item line and the place the evidence should be
-- the named harm, the count, the number -- and is not. If deciding
whether an item is product-shape, whether an RFC's text covers THIS
shape or main deliberately diverged from it, or whether the evidence
is really absent, takes judgement rather than reading, the exception
...
already exists, is aimed at the wrong level, changes the product's
shape with no recorded decision, or cannot be evaluated on the
evidence supplied.
...
grepped -- then the one-line subtraction that resolves it. A (c) or
(d) blocker names the missing record or evidence instead of a
subtraction; its title opens `product-shape change without accepted
RFC` or `cannot evaluate: missing <X>`.>
