...
Two new crew-home entries, each on BOTH surfaces the repo already ratchets:
the tool gate (``security._CREW_SECRET_LEAVES`` -> ``is_sensitive_path``) and
the OS sandbox (``sandbox._CREW_HIDDEN_LEAVES``):

* ``tasks/`` -- the durable task queue (``taskq/store.py``), other sessions'
  accepted work and the scheduler's authority. Hidden + fenced.
* ``logs/gatewayd_zombie_diagnostic.jsonl`` -- the gatewayd self-report the
  supervisor trusts before killing a daemon. Hidden + fenced.
...
The paths asserted are the ones the modules REALLY use (``TaskStore.default_path``,
``self_report.zombie_diagnostic_path``), so a moved file cannot silently leave
the fence behind.
"""
...
def _real_paths(scratch: Path | None = None) -> dict[str, str]:
    ...
    return {
        ...
        "self_report": _rebase(self_report.zombie_diagnostic_path(probe)),
    }


class TestToolGateFence:
    @pytest.mark.parametrize("key", ["tasks_db", "tasks_wal", "self_report"])
    ...
    def test_the_rest_of_logs_stays_readable(self) -> None:
        # Only the self-report leaf is fenced, not the operator's log directory.
        assert not security.is_sensitive_path(os.path.join(_CREW, "logs", "restart.log"))
        assert not security.is_sensitive_path(os.path.join(_CREW, "logs"))

    def test_the_leaves_are_declared_on_the_gate(self) -> None:
        leaves = set(security.paths._CREW_SECRET_LEAVES)
        assert {"tasks", "logs/gatewayd_zombie_diagnostic.jsonl"} <= leaves
        assert self_report.SELF_REPORT_FILENAME == "gatewayd_zombie_diagnostic.jsonl"


class TestSandboxDisposition:
    def test_tasks_and_the_self_report_are_hidden(self) -> None:
        hidden = set(sandbox._CREW_HIDDEN_LEAVES)
        assert "tasks" in hidden
        assert "logs/gatewayd_zombie_diagnostic.jsonl" in hidden
        for prefix in (".kiro/crew", ".kirocrew"):
            assert f"{prefix}/tasks" in sandbox._CREW_HIDDEN_DIRS
            assert f"{prefix}/logs/gatewayd_zombie_diagnostic.jsonl" in sandbox._CREW_HIDDEN_DIRS
    ...
    def test_the_launcher_masks_the_new_dirs(self, mode: str) -> None:
        ...
        assert os.path.join(_CREW, "logs", "gatewayd_zombie_diagnostic.jsonl") in masked
