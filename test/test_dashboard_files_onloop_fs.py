...
#: These are the sync seams the offload was introduced for; naming them here is
#: what stops a future edit from calling one directly again.
...
def _scan_own_body(node: ast.AsyncFunctionDef) -> list[str]:
    ...
    while stack:
        ...
        if isinstance(n, ast.Call):
            fn = n.func
            # ``await asyncio.to_thread(os.path.realpath, p)`` passes the
            # callable as an ARGUMENT, so it is not a Call node and is not
            # flagged -- which is correct: it runs off-loop.
...
class TestStaticRatchet:
    def test_no_guarded_endpoint_touches_the_filesystem_on_the_loop(self):
        ...
        assert not violations, (
            "blocking filesystem call(s) on the event loop. Offload via "
            "asyncio.to_thread, or move the call into a sync helper handed to "
            "one:\n" + "\n".join(violations)
        )
...
class _ThreadSpy:
    ...
    def assert_off_loop(self) -> None:
        ...
        on_loop = [t for t in self.seen if t == "MainThread"]
        assert not on_loop, (
            f"{len(on_loop)} of {len(self.seen)} filesystem call(s) on the "
            "request path ran on the event loop thread; they must run in a worker"
        )
...
class TestFileRead:
    ...
    async def test_validation_and_open_share_one_worker_transaction(
        ...
        assert threads["open"] != "MainThread"
...
class TestBrowseAndReveal:
    ...
    async def test_reveal_stat_and_launch_share_one_transaction(
        ...
        assert threads["launch"] != "MainThread"
