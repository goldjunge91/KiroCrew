...
class TestApprovalTimeoutInbandNotice:
    ...
    @pytest.mark.asyncio
    async def test_lost_claim_with_unresolved_click_falls_back_to_rejected(
        self, slack, monkeypatch
    ):
        """A click that claimed the entry but never resolves (hung wire write)
        must not park the turn: the bounded wait expires and the fabricated
        rejection stands."""
        monkeypatch.setattr(h, "_APPROVAL_TIMEOUT", 0)
        monkeypatch.setattr(h, "_LOST_CLAIM_RESOLVE_BOUND_SECS", 0.05)
        provider = _SteerRecordingProvider()

        async def _post(channel, blocks, text, thread_ts=None, **kw):
            ts = "9003.0"
            asyncio.get_running_loop().call_soon(
                lambda: h._pending_approvals.pop(f"{channel}:{ts}", None)
            )
            return ts
        ...
        assert outcome == h._OUTCOME_REJECTED
        # The hung click cannot be allowed to strand the request: the bound
        # expiring must answer the wire best-effort instead of returning with
        # the ACP permission still pending.
        assert ("reject_tool", "r1") in provider.calls
