...
def test_a_webex_space_entry_keeps_its_thread(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert route.user_id == "", "a space route carries no principal; it is authorized by room id"
...
def test_a_webex_allowed_space_gets_its_notice_at_the_room_root(spool_home: Path) -> None:
    ...
    with group rooms on) and grants there, so a space route -- which carries no
    principal -- is authorized by its ``room_id`` alone and the notice posts at
    the room root.
    """
    from kiro_crew.webex.transport import WEBEX_CAPABILITIES, WebexTransport

    transport = WebexTransport.__new__(WebexTransport)
    transport._allowed = frozenset()  # no sender principal; the room grants it
    ...
    transport.send_message = _send  # type: ignore[method-assign]
    # A space entry as the dispatch site writes it: room id, NO principal, NO thread.
...
def test_a_webex_revoked_space_gets_no_notice_from_an_allowed_sender(spool_home: Path) -> None:
    """Against the REAL Webex egress gate: a revoked space route is denied.

    ``may_send_to`` grants a space at the room-id arm (``room_id in
    _allowed_rooms``), then falls through to ``principal in _allowed``. A space
    route carries no principal, so a room absent from ``_allowed_rooms`` fails the
    room-id arm and leaves the fallthrough nothing to consult: the entry is
    DROPPED, even though the original sender is still allow-listed. The
    no-principal pairing is what stops that still-allowed sender from authorizing
    a revoked space through the fallthrough.
    """
    from kiro_crew.webex.transport import WEBEX_CAPABILITIES, WebexTransport
    ...
    transport._allow_group_rooms = True
    transport._allowed_rooms = frozenset()  # the room is revoked
    transport.capabilities = WEBEX_CAPABILITIES
    sent: list[Any] = []
    ...
    transport.send_message = _send  # type: ignore[method-assign]
    # A space entry as the dispatch site writes it: room id, NO principal.
    _spool(spool_home, channel_type="webex", conversation_id="SPACE9", text="please check CI")

    report = asyncio.run(replay_spooled(transports={"webex": transport}))

    assert report.dropped and not report.notified, "a revoked space was still noticed"
