"""Fairness lanes, the child reserve and the parent pauser (RFC overload-resilience §6, §13 Q3/Q5).
...
* ``SubagentManager`` admission -- the pump's lane order, the child reserve at
  cap 2 on a three-level tree, the adaptive lift, the pauser flag, and the
...
from kiro_crew.taskq import lanes, migrate, model
...
def test_schema_v3_backfills_lanes_on_a_v2_file(tmp_path: Path) -> None:
    path = tmp_path / "old.db"
    conn = sqlite3.connect(str(path))
    for stmt in migrate._SCHEMA_V1:
        conn.execute(stmt.replace(",\n      lane TEXT NOT NULL DEFAULT ''", ""))
    for stmt in migrate._SCHEMA_V2:
        conn.execute(stmt)
    conn.execute("INSERT INTO meta VALUES('schema_version', '2')")
    cols = [
        "id", "parent_id", "root_id", "session_key", "kind", "harness", "params_json",
        "scope_ref", "state", "created_at", "updated_at",
    ]  # fmt: skip
    rows = [
        ("r", None, "r", "dash:9", "subagent", "", "{}", "{}", "queued", 1.0, 1.0),
        ("c", "r", "r", "subagent:r", "subagent", "", "{}", "{}", "queued", 2.0, 2.0),
        ("k", None, "k", "cron:1", "subagent", "", "{}", "{}", "queued", 3.0, 3.0),
        ("u", None, "u", "cron_old", "subagent", "", "{}", "{}", "queued", 4.0, 4.0),
    ]
    conn.executemany(
        f"INSERT INTO tasks({', '.join(cols)}) VALUES({', '.join('?' for _ in cols)})", rows
    )
    conn.commit()
    assert {r[1] for r in conn.execute("PRAGMA table_info(tasks)")} >= {"wait_json"}
    assert "lane" not in {r[1] for r in conn.execute("PRAGMA table_info(tasks)")}
    assert migrate.apply_schema(conn) == 3
    got = dict(conn.execute("SELECT id, lane FROM tasks").fetchall())
    assert got == {"r": "dash:9", "c": "dash:9", "k": "system", "u": "system"}
    assert any(r[1] == "tasks_lane" for r in conn.execute("PRAGMA index_list(tasks)"))
    conn.close()
...
def test_fairness_settings_from_agent_config_clamps() -> None:
    ...
    assert s.parent_checkpoint_pause is True
...
def test_agent_config_declares_the_keys_with_defaults() -> None:
    ...
    assert cfg.parent_checkpoint_pause is False
...
# ── parent pauser: default off changes nothing ───────────────────────────────
...
class FakePauser:
    ...
    def can_pause(self, info: SubagentInfo) -> bool:
        return True

    async def pause(self, info: SubagentInfo) -> bool:
        self.paused.append(info.id)
        return True

    async def resume(self, info: SubagentInfo) -> bool:
        self.resumed.append(info.id)
        return True


def test_noop_pauser_satisfies_the_protocol() -> None:
    pauser: ParentPauser = NoopParentPauser()
    assert pauser.can_pause(SubagentInfo(id="x", task="t")) is False
    assert asyncio.run(pauser.pause(SubagentInfo(id="x", task="t"))) is False
    assert asyncio.run(pauser.resume(SubagentInfo(id="x", task="t"))) is True


@pytest.mark.asyncio
async def test_pause_flag_off_ignores_an_installed_pauser(h) -> None:
    hz = await h(max_concurrent=2, settings=FairnessSettings(parent_checkpoint_pause=False))
    fake = FakePauser()
    hz.mgr._parent_pauser = fake
    assert isinstance(hz.mgr._admission.parent_pauser(), NoopParentPauser)
    s = hz.spawn("S")
    await hz.settle()
    hz.block_in_spawn_sub_agents(s)
    a = hz.child_of(s, "A")
    await hz.settle()
    assert hz.state(s) == model.WAITING_CHILDREN
    assert fake.paused == [] and getattr(hz.live(s), "_paused", False) is False
    await hz.end(a)
...
@pytest.mark.asyncio
async def test_pause_flag_on_drives_the_installed_pauser(h) -> None:
    hz = await h(max_concurrent=2, settings=FairnessSettings(parent_checkpoint_pause=True))
    fake = FakePauser()
    hz.mgr._parent_pauser = fake
    assert hz.mgr._admission.parent_pauser() is fake
    s = hz.spawn("S")
    await hz.settle()
    hz.block_in_spawn_sub_agents(s)
    a = hz.child_of(s, "A")
    await hz.settle()
    assert hz.state(s) == model.WAITING_CHILDREN
    assert fake.paused == [s.id] and hz.live(s)._paused is True
    await hz.end(a)
    assert hz.state(s) == model.RUNNING
    assert fake.resumed == [s.id] and hz.live(s)._paused is False


@pytest.mark.asyncio
async def test_pause_flag_on_without_an_implementation_is_still_noop(h) -> None:
    hz = await h(max_concurrent=2, settings=FairnessSettings(parent_checkpoint_pause=True))
    assert isinstance(hz.mgr._admission.parent_pauser(), NoopParentPauser)
    s = hz.spawn("S")
    await hz.settle()
    ...
    await hz.settle()
    assert hz.state(s) == model.WAITING_CHILDREN
    assert getattr(hz.live(s), "_paused", False) is False
