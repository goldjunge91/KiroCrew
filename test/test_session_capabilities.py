...
async def test_new_runtime_adopts_and_busy_session_keeps_old_version(world):
    ...
    try:
        ...
        assert runtime_view(manager, "A", first["revision"])["status"] == "applied"
        ...
        view = runtime_view(manager, "A", latest["revision"])
        ...
        assert runtime_view(manager, "A", latest["revision"])["status"] == "applied"
...
async def test_startup_failure_never_applies_and_real_retry_works(world, fault):
    ...
    try:
        ...
        latest = await asyncio.to_thread(prepare_member_capabilities, "A", project)
        assert runtime_view(manager, "A", latest["revision"])["status"] == "failed"
...
async def test_runtime_status_tracks_process_and_handle_identity(world):
    ...
    try:
        ...
        assert runtime_view(manager, "A", prepared["revision"])["status"] == "unverified"
        provider.incarnation = original_process
        provider.sid = "new-handle-on-old-process"
        assert runtime_view(manager, "A", prepared["revision"])["status"] == "unverified"
        provider.sid = original_sid
        assert runtime_view(manager, "A", prepared["revision"])["status"] == "applied"
        manager._sessions["dashboard:A"].adopt_provider(provider)
        assert runtime_view(manager, "A", prepared["revision"])["status"] == "pending"
        manager.release("dashboard:A")
    finally:
        await manager.close_all(drain_timeout=0)
...
async def test_same_parent_members_get_distinct_runtime_versions(world):
    ...
    try:
        for member in ("A", "B"):
            ...
            view = runtime_view(manager, member, prepared["revision"])
...
@pytest.mark.parametrize("backend", ["", "kas", "claude", "codex"])
...
async def test_real_acp_mode_ack_is_required_for_loaded_template(tmp_path, resume):
        ...
        if resume:
            handle = await asyncio.wait_for(
                runtime.load_session(
                    str(tmp_path / "native.json"), "native-history-id", agent=template
                ),
                5,
            )
        else:
            handle = await asyncio.wait_for(runtime.create_session(agent=template), 5)
...
async def test_saved_bytes_changed_during_start_never_get_a_stamp(world):
    ...
    try:
        with pytest.raises(CapabilityError, match="materialization_changed"):
            await manager.get_or_create("dashboard:A", agent="A", cwd=str(project))
        assert not manager.has_session("dashboard:A")
        assert runtime_view(manager, "A", "unknown")["status"] == "failed"
    finally:
        await manager.close_all(drain_timeout=0)
...
async def test_runtime_application_requires_mcp_registration_evidence(world, monkeypatch):
    ...
    try:
        ...
        assert runtime_view(manager, "A", revision)["status"] == "unverified"
        ...
        failed = runtime_view(manager, "A", revision)
        ...
        assert runtime_view(manager, "A", revision)["status"] == "pending"
        report.record_event(EVENT_MCP_SERVER_INITIALIZED, "docs")
        assert runtime_view(manager, "A", revision)["status"] == "applied"
        report.record_unresolved_refs(["@missing"])
        assert runtime_view(manager, "A", revision)["status"] == "failed"
        assert provider.starts == 1 and provider.stops == 0
        manager.release("dashboard:A")
    finally:
        await manager.close_all(drain_timeout=0)
