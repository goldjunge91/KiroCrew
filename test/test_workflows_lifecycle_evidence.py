...
async def test_repeated_cancellation_cannot_publish_while_script_cleanup_is_alive(timeout):
    started, cleanup_started = asyncio.Event(), asyncio.Event()
    repeated_cancel, release = asyncio.Event(), asyncio.Event()
    script_tasks = []
    events = []

    async def agent_fn(prompt, opts):
        script_tasks.append(asyncio.current_task())
        started.set()
        try:
    ...
    runner = _runner(agent_fn=agent_fn, timeout_secs=0.01 if timeout else 3600)
    task = asyncio.create_task(
        runner.run(GOOD_SCRIPT, run_id="wf_repeat_cancel", now=NOW_SENTINEL, on_event=events.append)
    )
    try:
        await asyncio.wait_for(started.wait(), 5)
        if not timeout:
        ...
        task.cancel()
        await asyncio.wait_for(repeated_cancel.wait(), 5)
        assert not script_tasks[0].done()
        assert not task.done()
        assert not any(e.type in {"run_finished", "run_failed", "run_cancelled"} for e in events)
        release.set()
        result = await asyncio.wait_for(task, 5)
        assert script_tasks[0].done()
        assert result.events[-1].type == ("run_failed" if timeout else "run_cancelled")
        assert result.error == ("timeout" if timeout else "cancelled")
    finally:
        release.set()
        if not task.done():
            task.cancel()
