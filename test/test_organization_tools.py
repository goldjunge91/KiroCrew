...
def test_guarded_role_checks_the_spawn_resolvers_project_precedence(
    env, member_proof, team, monkeypatch, tmp_path, project_spec
):
    from kiro_crew import organization_policy
    ...
    if project_spec is not None:
        ...
        with pytest.raises(OrganizationError) as caught:
            organization_policy.prepare_agent("dashboard:alice", ACP_BACKEND_KIRO, project)
        assert caught.value.code == "organization_spec_shadowed"
    else:
        assert (
            organization_policy.prepare_agent("dashboard:alice", ACP_BACKEND_KIRO, project) == name
        )
