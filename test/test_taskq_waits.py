...
def test_schema_v2_adds_wait_column_and_upgrades_a_v1_file(tmp_path: Path) -> None:
    path = tmp_path / "old.db"
    conn = sqlite3.connect(str(path))
    v1 = [s for s in migrate._SCHEMA_V1]
    for stmt in v1:
        conn.execute(stmt.replace(",\n      wait_json TEXT", ""))
    conn.execute("INSERT INTO meta VALUES('schema_version', '1')")
    conn.commit()
    cols = {r[1] for r in conn.execute("PRAGMA table_info(tasks)")}
    assert "wait_json" not in cols
    assert migrate.apply_schema(conn) == migrate.SCHEMA_VERSION >= 2
    cols = {r[1] for r in conn.execute("PRAGMA table_info(tasks)")}
    assert "wait_json" in cols
