...
class TestDecidableFindingsExitTheTieBreaker:
    ...
    def test_first_principles_tie_breaker_exempts_the_rider_combination(self):
        ...
        # Four combinations are settled by reading, not by degree: (a) an
        ...
        # maintainer override, (d) a behaviour change whose required evidence
        # is absent.
        assert "Four combinations are settled by reading the diff" in contract
        assert "PRODUCT-SHAPE CHANGE WITHOUT A RECORDED DECISION" in contract
        assert "CANNOT EVALUATE -- REQUIRED EVIDENCE ABSENT" in contract
