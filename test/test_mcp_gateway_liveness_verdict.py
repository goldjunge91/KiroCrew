...
Sessions whose stub reconnect budget ran out during the outage lost the server
permanently.
...
async def test_one_escalated_miss_replaces_an_adopted_daemon(tmp_path, monkeypatch):
    ...
    gone, nothing is listening on its address and every stub is reconnecting
    against a socket that answers nobody. `stub._RECONNECT_TOTAL_BUDGET_SECS`
    gives them 60s before they give up and lose that MCP server for the rest of
    the session. One escalated miss displaces at about 22s, inside that budget;
    three 30s cycles would take about 126s, past it, turning a recoverable
    outage into permanent tool loss. The load misverdict is already handled by
    the escalation, so the patience would buy nothing and cost that.
    """
    m = _manager(tmp_path)
    m._adopted = True
    m._process = None
