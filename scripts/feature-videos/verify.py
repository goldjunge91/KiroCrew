...
def verify_folder(
    ...
    *,
    ...
    max_poster_bytes: int = DEFAULT_MAX_POSTER_BYTES,
    ...
    for entry in checked:
        ...
        for name, claimed in pairs:
            ...
            cap = max_clip_bytes if name == entry["file"] else max_poster_bytes
            if size > cap:
                raise ManifestError(f"{name} is {size} bytes, over the {cap} byte cap")

    manifest_digest, _ = hash_regular_file(folder / "manifest.json", where="release folder")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for name in ("max_clip_bytes", "max_poster_bytes", "max_payload_bytes", "max_document_bytes"):
