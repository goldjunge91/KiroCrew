...
_CREW_HIDDEN_LEAVES: tuple[str, ...] = (
    ...
    "tasks",
    # The gatewayd self-report leaf. A forged ``is_serving`` probe would keep a
    # dead daemon from ever being restarted; the daemon writes it and the
    # gateway reads it, both unsandboxed. Nested under ``logs/`` (the daemon's
    # path); the rename residual of a nested leaf is accepted here as for
    # ``apps/aws-control/data`` because a moved ``logs/`` also hides the file
    # the supervisor reads, which fails CLOSED (stale report = restart).
    "logs/gatewayd_zombie_diagnostic.jsonl",
