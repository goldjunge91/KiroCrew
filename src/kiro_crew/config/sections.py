...
class AgentConfig:
    ...
    parent_checkpoint_pause: bool = field(
        default=False,
        metadata=_meta(
            "Parent Checkpoint Pause",
            "When a dedicated-runtime parent yields its slot to wait on its "
            "children, also close its runtime at a checkpoint and resume it "
            "through the continuable-session path when the children finish, so "
            "waiting parents stop charging the host budget. Off by default: the "
            "ParentPauser interface is wired but no runtime is paused, and a "
            "waiting parent keeps its resident process. Turn on only once "
            "session/load fidelity for a mid-tool-call parent is proven.",
        ),
    )
...
class McpGatewayConfig:
    ...
    spawn_queue_wait_secs: int = field(
        ...
        metadata=_meta(
            ...
            "is refused for capacity. Matches the stub's own reconnect budget: for "
            "that long kiro-cli's transport stays open and the server's tools stay "
            "listed. A refusal after this is reported to the session as a typed "
            "error naming the class and a retry hint, never as a crashed server.",
            restart=True,
        ),
    )
