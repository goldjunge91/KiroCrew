...
from kiro_crew.mcp_gateway import credwatch, hazards
from kiro_crew.mcp_gateway import self_report as _self_report
from kiro_crew.mcp_gateway import socketsec, tool_surface, transport
...
async def _handle_connection(
    ...
    *,
    ...
    # read again, so nothing kiro-cli sent during a respawn is dropped.
...
# Interval between diagnostic snapshots. Owned by ``self_report`` (a leaf module
# both this daemon and its supervisor import) because the supervisor's watchdog
# reads the records this loop writes and judges their freshness against this
# very interval: two independent literals would let the reader call a live
# daemon's report stale. Re-exported under the old name so tests can still
# monkey-patch it here.
_ZOMBIE_PROBE_INTERVAL_SECS = _self_report.ZOMBIE_PROBE_INTERVAL_SECS


def _zombie_diagnostic_path() -> Path:
    """Return the JSONL file path that receives zombie post-mortems.

    Delegates to :func:`self_report.zombie_diagnostic_path` -- the same
    resolver the supervisor reads through -- so writer and reader can never
    spell the path differently. This module's ``_config_dir`` seam still
    chooses the base directory.
    """
    return _self_report.zombie_diagnostic_path(_config_dir())
