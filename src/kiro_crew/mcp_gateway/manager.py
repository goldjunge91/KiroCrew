...
class GatewayManager:
    ...
    async def _ping_raw(self, *, timeout: Optional[float] = None) -> Optional[dict]:
        ...
        listening on that address until a replacement lands, and
        ``stub._RECONNECT_TOTAL_BUDGET_SECS`` gives attached stubs 60s before
        they lose that server for the session's life. Per step the escalated
        probe alone reaches 3 x 20s, and with the fast probe's own 3 x 2s ahead
        of it the worst case is 66s, past that budget. Shared, the pair is
        bounded at 26s, which leaves room for the up-to-30s notice latency
        before it.
        ...
        while not self._stopping:
            ...
            if proc is None:
                if self._adopted:
                    ...
                    # have different deadlines. If an adopted daemon really is
                    # gone, nothing is listening on its address, and every stub
                    # is reconnecting against a socket that answers nobody;
                    # `stub._RECONNECT_TOTAL_BUDGET_SECS` gives them 60s before
                    # they give up and lose that MCP server for the rest of the
                    # session. One escalated miss displaces at about 22s, inside
                    # that budget. Three cycles would take about 126s, past it,
                    # so a grace period here would convert a recoverable outage
                    # into permanent tool loss for every attached session.
