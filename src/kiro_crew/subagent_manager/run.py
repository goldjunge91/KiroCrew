...
class RunEventCoordinator(ManagerComponent):
    ...
    async def _run_inner_impl(
        ...
        # schedule or falls back to the in-turn ladder.
        _dep_coordinator = self._manager._dependency_coordinator()
    ...
    async def _yield_for_dependency_impl(self, info: SubagentInfo, signal: Any) -> bool:
        ...
        coordinator = self._manager._dependency_coordinator()
        if coordinator is None or info.done or info._reap_started or info.user_stopped:
            return False
        # A provider error can land before the turn's first frame (the prompt
        # itself was refused). The session IS live -- the turn was issued -- so
        # the row is ``running`` now; a ``starting`` row would be PARKED
        # (``retry_wait``, re-dispatched at the deadline) instead of waiting
        # under this run's lease.
        self.ensure_running_marked(info)
        ...
        verdict = coordinator.report(
            info.id, signal, generation=info._taskq_generation or None, from_state="running"
        )
    ...
    def ensure_running_marked(self, info: "SubagentInfo") -> None:
        ...
        Normally done by the first stream event; the wait paths call it too,
        because a wait can only be recorded on a ``running`` row and the turn
        that failed was a real turn on a live session.
        """
    ...
    def _taskq_note_stop_recovery_impl(self, info: SubagentInfo, data: dict[str, Any]) -> None:
        ...
        if store is None:
            return
        try:
            store.append_event(info.id, "stop_recovery", data)
            store.record_progress(info.id, info._taskq_generation, dict(data))
        except Exception:
            logger.debug("taskq: stop_recovery note for %s failed", info.id, exc_info=True)
