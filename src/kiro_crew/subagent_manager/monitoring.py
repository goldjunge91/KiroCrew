...
class OrphanStallMonitor(ManagerComponent):
    ...
    def taskq_pump(self) -> None:
        ...
        if loop is not None and store is not None and type(admission).pump_off_loop:
            # Timer callback on the loop: the expiry sweep's store calls run
            # on the writer thread; its cancellations land back on the loop.
            live_store = store
        ...
        deadline = coordinator.next_deadline()
        if deadline is None:
            return
