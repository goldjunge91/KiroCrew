...
run on their own module's globals and may import ``taskq`` at the top.
...
* :mod:`.types` -- the value types (settings, prepared row, claim point,
  capacity reading, parent pauser) and the constants importers read.
...
* :mod:`.fairness` -- lanes, the child reserve, the capacity view, the parent
  pauser.
