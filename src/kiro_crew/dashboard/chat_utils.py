...
async def drained(coro):
    ...
    mutation whose steps are themselves awaits (a hire is create → copy →
    link, with a roll-back on failure), a cancellation landing between two
    steps -- a gateway shutdown, a client that closed the request -- would
    leave the first step committed and the rest never run: a member bound to
    the shared source it was told it owns, with no roll-back. Shielding the
