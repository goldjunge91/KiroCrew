...
SET_SECTIONS = ("tools", "allowedTools", "skills", "resources")
MAX_OPERATIONS = 300
...
class CapabilityService:
    ...
    def _project(
        ...
        if not accepted <= {(i["section"], i["id"]) for i in changes}:
        ...
        return safe_view(
            {
                ...
                "runtime": runtime,
                "warnings": [snap["error"]] if snap["error"] else [],
            }
        )
    ...
    def publish(self, member: str, expected_template: str, name: str) -> dict:
        ...
        if (
            ...
            or name.split(".", 1)[0].upper() in {"CON", "PRN", "AUX", "NUL"}
...
def reconcile_member_capabilities(member: str) -> None:
    ...
    expected = {saved.get("materialized")}
    if saved.get("status") == "pending":
        expected.add(saved.get("previous_materialized"))
    if _digest(snapshot["spec"]) not in expected:
        raise CapabilityError("materialization_changed")
