...
def runtime_view(manager: Any, member: str, saved_revision: str) -> dict[str, Any]:
    """Read actual registry occupants on the event loop; no IO or runtime mutation."""
    rows = []
    ...
    for key, attempt in manager._allocation_boundary().state.capability_failures.items():
        if attempt["member"] == member and key not in manager._sessions:
