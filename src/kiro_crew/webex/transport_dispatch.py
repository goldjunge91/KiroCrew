        ...
        try:
            await drive_turn(
                ChannelTurn(
                    ...
                    # the thread and NO principal. ``may_send_to`` checks the
                    # room-id arm FIRST (``room_id in _allowed_rooms``) and grants
                    # there, then falls through to ``principal in _allowed``. So a
                    # space route is authorized by its ``room_id`` alone -- delivered
                    # while the room is allow-listed, denied once it is revoked
                    # because the route carries no principal for the fallthrough. A
                    # space route that carried a principal would let a still-allowed
                    # sender authorize a revoked space through that fallthrough, and
                    # a DM route that carried a thread would be read as threaded and
                    # denied for want of a principal. ``text`` is the mention-
                    # stripped original, not the ingested prompt.
