...
def rederive_worker_agent(reason: str) -> bool:
    ...
    Public and named because it now has two callers with nothing else in common --
    the boot install path and the app registration transaction -- and because the
    next writer of ``kirocrew.json`` needs one obvious thing to call rather than a
    reason to rediscover this. Takes only a *reason* string, for the log: a caller
    that had to hand over a config or a path would be a caller that could hand over
    the WRONG one, and the whole point of the derivation is that it reads the
    installed default itself.
    ...
    **Must not be called while holding ``bridges._mcp_lock``.** The installer takes
    ...
    already would invert the order this module establishes. Every registration path
    calls this after its own MCP transaction has committed and released.
