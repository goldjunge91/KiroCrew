...
class DependencyCoordinator:
    ...
    def tick(self) -> list[str]:
        ...
        never delayed by a throttled one.
        """
        now = self.now()
        woken: list[str] = []
        with self._lock:
            for scope in sorted(self._scopes):
                ...
                if self._deadline_passed(sched, now) and sched.phase == PHASE_WAITING:
                    self._fail_scope(
                        sched, f"dependency {scope} unavailable for {now - sched.since:.0f}s", now
                    )
                    continue
                ...
                for task_id in list(sched.waiters)[:batch]:
                    ...
                    self._wake_one(task_id, generation, sched, now)
    ...
    def _wake_one(
        self, task_id: str, generation: int | None, sched: ScopeSchedule, now: float
    ) -> None:
    ...
    def _fail_scope(self, sched: ScopeSchedule, reason: str, now: float) -> None:
