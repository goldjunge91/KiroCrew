...
* :func:`apply_schema` brings a connection to ``SCHEMA_VERSION`` -- creating the
  tables on an empty database and stepping an older one forward. It runs on
  every open, so a downgrade-then-upgrade leaves nothing to do by hand.
...
#: v2: the ``WaitRecord`` column and the parent index the wake-by-child
#: lookup needs. A v1 database gets ``ALTER TABLE ADD COLUMN``; a fresh one
#: already has the column from the v1 CREATE above (which is the current
#: shape), so the ALTER is guarded by a column probe.
_SCHEMA_V2 = ("CREATE INDEX IF NOT EXISTS tasks_parent ON tasks(parent_id, state)",)

#: v3: the fairness ``lane`` column and the index the per-lane dispatch query
#: reads. Existing rows are backfilled: automation roots to ``system``, other
#: roots to their own session key, nested rows to their root's lane.
_SCHEMA_V3 = (
    "CREATE INDEX IF NOT EXISTS tasks_lane ON tasks(lane, state, next_run_at, created_at)",
)
_BACKFILL_V3_ROOTS = (
    "UPDATE tasks SET lane = CASE "
    "WHEN session_key = '' OR session_key IN ('_hb', '_bg') "
    "OR session_key LIKE 'cron:%' OR session_key LIKE 'cron\\_%' ESCAPE '\\' "
    "OR session_key LIKE 'hook:%' OR session_key LIKE 'webhook:%' THEN 'system' "
    "ELSE session_key END "
    "WHERE lane = '' AND (parent_id IS NULL OR parent_id = '')"
)
_BACKFILL_V3_NESTED = (
    "UPDATE tasks SET lane = COALESCE("
    "(SELECT r.lane FROM tasks r WHERE r.id = tasks.root_id AND r.lane <> ''), "
    "session_key) "
    "WHERE lane = '' AND parent_id IS NOT NULL AND parent_id <> ''"
)
...
def apply_schema(conn: sqlite3.Connection) -> int:
    """Create or upgrade the schema; returns the version now in force.

    Runs inside one transaction so a crash mid-upgrade leaves the previous
    version intact rather than a half-built one. A database NEWER than this
    code understands is refused: writing rows an older writer does not know the
    columns of is how a downgrade corrupts a store.
    """
    ...
    try:
        ...
        if current < 2:
            if not _has_column(conn, "tasks", "wait_json"):
                conn.execute("ALTER TABLE tasks ADD COLUMN wait_json TEXT")
            for statement in _SCHEMA_V2:
                conn.execute(statement)
        if current < 3:
            if not _has_column(conn, "tasks", "lane"):
                conn.execute("ALTER TABLE tasks ADD COLUMN lane TEXT NOT NULL DEFAULT ''")
            for statement in _SCHEMA_V3:
                conn.execute(statement)
            conn.execute(_BACKFILL_V3_ROOTS)
            conn.execute(_BACKFILL_V3_NESTED)
        # Future versions: ``if current < 4: ...`` steps land here, in order.
        conn.execute(
