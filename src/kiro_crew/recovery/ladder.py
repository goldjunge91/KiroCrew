...
L4      gatewayd        liveness ping failed 3x AND no          daemon drain       1 / 10min  L5
                        self-report AND no backend progress                        (2nd in
                                                                                   window)
...
Every layer reads its schedule from the one :class:`RecoveryPolicy` in
:data:`LADDER`, so a change to the base or the cap moves every layer together
and the layers stay de-correlated by jitter rather than by luck.
...
LADDER: RecoveryPolicy = RecoveryPolicy(
    layers={
        ...
        L4_GATEWAYD: LayerPolicy(
            layer=L4_GATEWAYD,
            trigger=(
                "liveness ping failed 3x AND self-report absent/stale AND no "
                "backend progress for 60s"
            ),
