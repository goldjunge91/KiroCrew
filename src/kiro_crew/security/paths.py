...
_CREW_SECRET_LEAVES: list[str] = [
    ...
    "tasks",
    # The gatewayd self-report (mcp_gateway/self_report.py): the daemon's own
    # ``is_serving`` probes, which the supervisor consults before killing a
    # daemon that stopped answering pings. A forged probe keeps a dead daemon
    # alive forever (every session's MCP transport with it); written by the
    # daemon, read by the gateway -- both open the path directly. Nested under
    # ``logs/`` because that is where the daemon writes it.
