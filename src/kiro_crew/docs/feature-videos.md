...
result with the release key, and writes `manifest.json` and `SHA256SUMS` into the
same folder. `scripts/feature-videos/verify.py` re-checks a folder before upload,
and a human runs the upload. Neither command touches the network, so the
