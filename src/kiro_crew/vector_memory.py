        ...
        *facets* stamps the crew lineage's carve axes and is ignored on v1. An
        ...
        with self._db_lock:
            ...
            if preserve_existing:
                ...
                mem_id = str(uuid4())
                now = _now_iso()
                # An explicit immediate transaction (rather than the connection's
                # implicit deferred one) so the signature re-read and the insert
                # cannot be interleaved by another process's reconcile.
                self.db.execute("BEGIN IMMEDIATE")
                try:
                    if not self._supplied_vector_admissible(embedding_blob, embedding_space):
                        embedding_blob = None
