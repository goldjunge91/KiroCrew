...
class VectorMemoryStore:
        ...
        *,
        ...
        if self.embed_fn is not None and not key.startswith("lesson.") and not already_embedded:
            ...
            if vec:
                ...
                with self._db_lock, self._vector_commit(vec, best_effort=True) as current:
    ...
    def write_episodic(
        ...
        *facets* stamps the crew lineage's carve axes and is ignored on v1. An
        ...
        with self._db_lock, self._embedding_config_guard(embedding):
    ...
    def write_lesson(
        ...
        *,
        ...
        if rule_emb:
            ...
            with self._db_lock, self._vector_commit(rule_emb, best_effort=True) as current:
    ...
    def _vector_commit(self, vector: list[float] | None, *, best_effort: bool = False):
        ...
        with self._db_lock, ExitStack() as resources:
            started = False
        ...
        *pace* (default on) idles between rows so the sweep targets
        ...
        for row in rows:
            ...
            if norm > 0:
                arr = arr / norm
            blob = arr.tobytes()
            with self._db_lock, self._vector_commit(vec) as current:
                if should_stop is not None and should_stop():
                    break
